...
public class AccountController extends AbstractContextAwareController implements Serializable {
	...
	//Chaque status contient une liste de champs de validation spÃ©cifiques
	...
	public String pushValid() {
		try {
			...
			logger.debug("La validation concerne les donnï¿½es suivantes: "+hashInfToValidate.toString());
			...
			if (currentAccount.getAttribute(accountCodeKey)!=null) {
				...
				if (reinit){
					...
					if (beanFieldChannels.size()>1){						
						...
						logger.debug("Code envoyÃ© via le canal : "+beanFieldChannels.get(0).getValue());
						return "gotoPushCode";
					}
					else{
	 ...
	 * 
	 * @param beanPersoInfo
	 * @return indique si ce champ a Ã©tÃ© modifiÃ© ou non
	 */
	...
	public String pushChoice(){
		try{
			...
			logger.debug("Code demandÃ© par utilisateur pour le recevoir sur le canal : "+oneChoiceCanal);
			return "gotoPushCode";
